#Python program for reading and writing into excel
# using openpyxl downloaded from pypi
...
logger = logging.getLogger("ExcelFileDemo")
...
""" Problem 1: Count and display number of supplier and their product count
    ...
    Problem 4: Add a new column and update with the price of each product inventory """
...
print(f"Inventory value per supplier in $: {inventory_value_per_supplier}")
print(f"Products under 10 : {products_under_10_inv}")

print("Saving output by adding inventory cost for each product")
result = bool(1)
try:
    inventory_file.save("myInventorySolution.xlsx")
except:
    logger.error("ERROR while saving the output file")
    result = bool(0)
finally:
    print(f"Result of the program is {result}. Terminating.")